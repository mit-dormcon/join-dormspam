<script lang="ts">
	...
	const mailmanWarning =
		'Please note that this email is being sent to a bot that ends with -request@mit.edu. NEVER EVER email the actual list with something like subscribe or unsubscribe, because that goes to everyone. This is fine because it ends with -request and is a bot.';
	let mailmanStep1 = $state(false);
	let mailmanStep2 = $state(false);
	let subscriptionComplete = $derived(mailmanStep1 && mailmanStep2);
</script>
...
{:else}
	<p>
		You are in <strong>{dormInfo.friendlyName}</strong>. To subscribe or unsubscribe from dormspam,
		please go to
		<a href={getMailmanLink(dormInfo.listName)} target="_blank">{dormInfo.listName}</a>. There are
		two main steps to join a mailman list:
	</p>

	<p>
		<label>
			<input type="checkbox" bind:checked={mailmanStep1} />
			I have requested a subscription
			<a href={getMailmanLink(dormInfo.listName)} target="_blank">on Mailman</a>
	...
	<iframe src={getMailmanLink(dormInfo.listName)} title="mailman" width="100%" height="100%"
	></iframe>
{/if}
